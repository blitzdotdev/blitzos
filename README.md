...
Your Claude subscription includes free cloud computers. Every task you start at [claude.ai/code](https://claude.ai/code) gets a fresh VM that clones your repos, runs code and tests, pushes branches, and opens PRs — and it keeps working after you close the tab. From a browser or a phone. No API bill.

The catch: **every new VM boots knowing nothing.** It doesn't know which repos belong together, how they relate, what your conventions are, or what the last session already did. So most people re-explain their setup every single session — or treat cloud sessions as a toy.

BlitzOS fixes the boot. It builds you a **context repo** — one private GitHub repo that teaches every new computer your whole setup:
...
your-context-repo/
  ...
  sessions/      # every session commits a record of what it did
  docs/          # optional power-mode setup
```

Launching becomes one link: the VM boots with the context repo plus every work repo selected, reads the map, reads what previous sessions did, and starts warm. Your laptop can be off the entire time.
...
**Browser (fastest):** sign in with GitHub at [blitzos.com](https://blitzos.com), pick the repos that belong together, and BlitzOS creates the context repo and gives you one-click launches — plus a feed that tracks every session's live status.
...
- **Warm multi-repo context** — sessions start knowing your conventions, the repo map, and what previous sessions did.
...
- **A session feed** — [blitzos.com](https://blitzos.com) shows every session's live status (working / quiet / done) with a link to open any of them, from any device. One 2-minute environment setup.
- **Your connectors** — sessions can use Linear, Slack, Gmail, and whatever else is already connected to your claude.ai account.
- **A shared work log** — each session records PRs and decisions in `sessions/`, so work compounds instead of evaporating.
...
- [ ] **Self-updating context** — sessions propose PRs back to the context repo when they learn something (a new convention, a decision, a map change), so context compounds instead of rotting
- [ ] **Codex support** — the same context repo boots OpenAI Codex cloud sessions; your context stops being vendor-locked
...
- [ ] **Real tools, full loops** — sessions get the same tools a person in that role would have: email, Slack, Linear, analytics, Stripe. Read the ticket, check the code, ship the fix, reply to the customer, close the ticket — one session.
- [ ] **Key-card sessions** — each session is a fork of your company containing only the repos, connections, and secrets its job needs. Everything it touches is audit-logged.
